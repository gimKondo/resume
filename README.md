...
ただし、3～10人程度の比較的小さなチームでの開発が多く、ほとんどのケースでクラウドインフラの構築やフロントエンドの開発を兼務していました。結果として、サーバサイド中心としてクラウドインフラからフロントエンドまで対応できるスキルが付きました。
...
これらの実績から、現在はチーム横断的な開発プロセスの改善を実施する職務に就いています。
...
プログラミングはほぼ未経験で入社したため、OJTでプログラミングの基礎を教わりながらCADのカーネル部の実装を担当。
OJTで非常に良い師を持てたこともあり、コーディング、コンピュータのアーキテクチャなどを深く学んだ。
CADのカーネルという性質上、メモリの効率的な利用が求められるため、低レベルな領域の知識を身に付けた。
